...
export const lectureApi = {
  // 강의 목록 조회
  async getLectures(options: LectureListOptions = {}): Promise<ApiResponse<Lecture[]>> {
    await delay();
  ...
  // 강의 생성 (강사만)
  async createLecture(lectureData: CreateLectureRequest): Promise<ApiResponse<Lecture>> {
    await delay();
    ...
    if (lectureData.maxStudents < 1) {
      return { success: false, message: '최대 수강 인원은 1명 이상이어야 합니다.' };
    }
  ...
  // 강의 수정 (강사만, 본인 강의만)
  async updateLecture(lectureData: UpdateLectureRequest): Promise<ApiResponse<Lecture>> {
    await delay();
    ...
    const updatedLecture = mockStorage.updateLecture(lectureData.id, lectureData);
  ...
  async cancelApplication(lectureId: string): Promise<ApiResponse<void>> {
    ...
    const success = mockStorage.cancelLectureApplication(lectureId, session.user.id);
  ...
  sortLectures(lectures: Lecture[], sortBy: LectureSortOption): Lecture[] {
    switch (sortBy) {
      ...
      case 'popular':
        return [...lectures].sort((a, b) => b.currentStudents - a.currentStudents);
